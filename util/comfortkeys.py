...
class ConfortKeys:
    ...
    def comfortIndex(self, key):
        """ returns the comfort index for key """
        return self.keys[key]

    def lowestIndex(self):
        """ returns the lowest comfort index """
        return min(self.keys.values())

    def highestIndex(self):
        """ returns the highest comfort index """
    ...
    def scoreLayout(self, layout, frequency):
        ...
        this function computes the comfort score based on the
        comfort key indexes """
        score = 0.0
        for k in self.keys:
